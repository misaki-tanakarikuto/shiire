...
if (!$link) {
    die('接続失敗です。'.pg_last_error());
		...
		foreach($customerArr as $customer){
			...
			/***** 購入情報品目テーブル検索 *****/
			/************************************/
			$sqlWhere = " where hinshu_cd = ".$customer."";
			...
			if($cstm_dt[0]){
				...
				$sqlString = "delete from mst_hinshu where hinshu_cd = " . $customer . ";";
				$result = pg_query($sqlString);

				if (!$result) {
					error_log("クエリーが失敗しました（品種サイズマスタデータ削除）sql=\n" . $sqlString . "\n" , 3, 'c:/WWW/siire/log/app.log');
					die('クエリーが失敗しました（品種サイズマスタデータ削除）'.pg_last_error());
				}
				//削除した
				$delele_cnt++;
