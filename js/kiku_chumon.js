...
//　注文データを画面へセット
//*****************************************************************************
function chumonDataSet(){
	//ここに来た時点でここで注文データは存在する。
	//来る前に判定している。
	//各部署には、品目サイズが"２Ｌ"、"Ｌ"、"Ｍ"の３つのレコードが
	//必ず存在する。例えば２Ｌの注文が無い場合は注文ゼロで追加している。
	//「注文本数」の値も計算して配列に入れている。
	//注文本数の合計行（一覧の一番下の行）用のレコードも部署別に
	//配列に入れている（himmoku_size_cd=0のレコードで、そのレコードには
	//「busho_honsu_gokei」という列があり、そこに部署ごとの注文本数合計値が
	//入っている。
	//PHP側でその処理を行っているよ。

	//「注文購入日」をセット
	// 字列.replace(置換する文字列, 新しい文字列)
	// var wk_chumon_konyu_date = new Date(chumon_arr[0]["chumon_konyu_date"]);
	// wk_chumon_konyu_date = formatDate(wk_chumon_konyu_date, "YYYY/MM/DD");
	// document.getElementById("chumon_konyu_date").value = wk_chumon_konyu_date;

	var tbl = document.getElementById("tbl_lst");	//テーブルオブジェクト
...
function chgDate(){
	...
	var obj = document.getElementById("disp_chumon_konyu_date");	//プルダウンで選択されている日付を取得
	var disp_chumon_konyu_date = obj.options[obj.selectedIndex].value;	// プルダウンで選択した社員
